...
def get_json_file(hashed_email: str, endpoint: str) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"File '{key}' is not valid gzip data"
        )
    except json.JSONDecodeError as e:
        raise HTTPException(
            status_code=500, detail=f"File '{key}' is not valid JSON: {e}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Unexpected error reading '{key}': {e}"
        )
...
def save_json_file(hashed_email: str, endpoint: str, data) -> None:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Unexpected error saving '{key}': {e}"
        )
...
def ensure_json_file_exists(hashed_email: str, endpoint: str) -> None:
    ...
    try:
        ...
        if error_code == "404":
            ...
            raise HTTPException(
                status_code=500, detail=f"Unexpected S3 error checking '{key}': {e}"
            )
...
def upload_pdf_to_s3(buffer, hashed_email, filename):
    ...
    try:
        s3.upload_fileobj(
            buffer, bucket_name, s3_key, ExtraArgs={"ContentType": "application/pdf"}
        )
        return s3_key
    except ClientError:
        raise
...
def upload_id_to_s3(file_obj, s3_key) -> bool:
    ...
    try:
        s3.upload_fileobj(
            file_obj, bucket_name, s3_key, ExtraArgs={"ContentType": "application/pdf"}
        )
