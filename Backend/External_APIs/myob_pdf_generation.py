"""
MYOB PDF Generators - Returns PDFs as bytes for S3 upload
Usage:
    from myob_pdf_generators import generate_payroll_pdf, generate_sales_pdf, generate_banking_pdf, generate_purchases_pdf
    
    all_results = retrieve_endpoints_myob(access_token, business_id)
    
    payroll_pdf_bytes = generate_payroll_pdf(all_results)
    sales_pdf_bytes = generate_sales_pdf(all_results)
    banking_pdf_bytes = generate_banking_pdf(all_results)
    purchases_pdf_bytes = generate_purchases_pdf(all_results)
    
    # Upload to S3
    s3_client.put_object(Bucket='your-bucket', Key='payroll.pdf', Body=payroll_pdf_bytes)
"""
...
def generate_payroll_pdf(all_results):
    ...
    advice_data = find_data_by_endpoint(all_results, "Report/Payroll/EmployeePayrollAdvice")
...
def generate_sales_pdf(all_results):
    ...
    # Invoices table
    if all_invoices:
        rows = [[inv["Date"], inv["Number"], short(inv["CustomerName"], 32), inv["Status"], 
                 money(inv["Total"]), money(inv["BalanceDue"]), inv["DueDate"] or "-"] 
        ...
        story.append(Paragraph("Most recent 100 invoices. Balance = Total - Paid.", styles["caption"]))
        story.append(Spacer(1, 6))
    ...
    if payment_data.get("Items"):
        ...
        rows = [[p["Date"], short(p["CustomerName"], 35), money(p["Amount"]), p["Memo"]] 
                for p in payments]
        ...
        story.append(Paragraph("Last 100 payments. Supports income stability proof.", styles["caption"]))
        story.append(Spacer(1, 6))
    ...
    if refund_data.get("Items"):
        ...
        rows = [[r["Date"], short(r["CustomerName"], 35), money(r["Amount"]), 
                 r["InvoiceNumber"], short(r["BankAccount"], 40)] for r in refunds]
        ...
        story.append(Paragraph("Refunds reduce net income. Shown for transparency.", styles["caption"]))
...
def generate_banking_pdf(all_results):
    ...
    if accounts_data.get("Items"):
        rows = [[acc.get("BankAccountName", "Unknown"), mask(acc.get("BankAccountNumber", "")),
                 acc.get("FinancialInstitution", "Unknown"), (acc.get("LastReconciledDate") or "")[:10]]
        ...
        story.append(Paragraph("Account numbers masked for security. Full details in MYOB.", styles["caption"]))
        story.append(Spacer(1, 6))

    # Statements
    stmt_data = find_data_by_endpoint(all_results, "Banking/Statement")
    if stmt_data.get("Items"):
        rows = [[(ln.get("Date") or "")[:10], short(ln.get("Description", ""), 70),
        ...
        story.append(Paragraph("Full history shown. Positive = deposit, Negative = withdrawal.", styles["caption"]))
        story.append(Spacer(1, 6))

    # Received money
    recv_data = find_data_by_endpoint(all_results, "Banking/ReceiveMoneyTxn")
    if recv_data.get("Items"):
        rows = [[(txn.get("Date") or "")[:10], short((txn.get("Contact") or {}).get("Name", "Unknown"), 50),
                 money(txn.get('AmountReceived', 0)), txn.get("PaymentMethod", "Unknown")]
        ...
        story.append(Paragraph("All money received into accounts. Supports income proof.", styles["caption"]))
        story.append(Spacer(1, 6))
    ...
    if spend_data.get("Items"):
        rows = [[(txn.get("Date") or "")[:10], short((txn.get("Contact") or {}).get("Name", "Unknown"), 50),
                 money(txn.get('AmountPaid', 0)), txn.get("PaymentMethod", "Unknown")]
        ...
        story.append(Paragraph("All payments made. Helps verify expenses and cash flow.", styles["caption"]))
...
def generate_purchases_pdf(all_results):
    ...
    all_bills = []
    for suffix in ["Purchase/Bill", "Purchase/Bill/Item", "Purchase/Bill/Professional", "Purchase/Bill/Service"]:
        data = find_data_by_endpoint(all_results, suffix)
        for it in (data.get("Items") or []):
    ...
    # Bills overview
    if all_bills:
        rows = [[b["Date"], short(b["SupplierName"], 35), b["Status"], money(b["TotalAmount"]),
                 money(b["AppliedToDate"]), money(b["BalanceDueAmount"]), b["DueDate"] or "-"]
                for b in all_bills]
        ...
        if line_rows:
            ...
            story.append(Paragraph("Truncated for readability. Full details available in MYOB exports.", styles["caption"]))
