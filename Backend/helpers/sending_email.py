...
def dukbill_style_html_accountant(
    broker_name: str,
    broker_email: str,
    client_first_name: str,
    msg_contents: str,
    ...
    """
    Generates a Dukbill-style HTML email for client-to-accountant communication.
    Follows the same design as dukbill_style_html but tailored for accountant interactions.
    """
...
<head>
  ...
  <meta name="x-apple-disable-message-reformatting">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Accountant Message</title>
</head>
<body style="margin:0;padding:0;background-color:#f4f6f8;">
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background-color:#f4f6f8;">
    <tr>
...
def send_client_to_accountant(
    ...
    msg = EmailMessage()
    msg["From"] = f"{accountant_name} <{from_email}>"
    msg["Reply-To"] = accountant_email  # so replies go to the accountant
    msg["To"] = client_email
    msg["Subject"] = subject or f"Message from {accountant_name}"
