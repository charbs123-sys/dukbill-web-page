...
from Database.S3_init import bucket_name, s3
...
from helpers.helper import hash_email, truncate_pdf
...
def get_client_dashboard(client_id: str, emails: list) -> list:
    ...
    for doc in all_documents:
        ...
        myob_reports = doc.get("myob_reports", {})
        for filename, broker_comment in myob_reports.items():
            myob_map.setdefault(filename, []).append(
                {
                    "id": filename,
                    "category_data": [],
                    "hashed_email": doc.get("hashed_email"),
    ...
    # xero headings - organization name as category
    if xero_map:
        xero_categories = [
            {"category_name": org_name, "cards": cards}
            for org_name, cards in xero_map.items()
        ]
    ...
    if myob_map:
        myob_categories = [
            {"category_name": category_name, "cards": cards}
...
def update_anonymized_json_myob(
    hashed_email: str, parent_header: str, sibling_header: list[str]
) -> bool:
    """
    updating anonymized json for myob and xero
    ...
    for doc in documents:
        if parent_header in doc:
            ...
            doc[parent_header] = {filename: "" for filename in sibling_header}
            save_json_file(
                hashed_email, "/broker_anonymized/emails_anonymized.json", documents
            )
            return True
    
    # If parent_header doesn't exist, create new entry
    documents.append(
        {parent_header: {filename: "" for filename in sibling_header}}
    )
    save_json_file(hashed_email, "/broker_anonymized/emails_anonymized.json", documents)
    return True


def get_docs_myob(client_id: str, emails: list, filename: str) -> list:
    """
    Get MYOB documents for a specific filename.
    
    Args:
        client_id: The client identifier
        emails: List of email addresses
        filename: The specific report filename (e.g., "Broker_Payroll_Summary.pdf")
    ...
    for doc in documents:
        ...
        if filename in myob_reports:
            broker_comment = myob_reports[filename]
            
            try:
                s3_key = f"{hashed_email}/myob_reports/{filename}"
                url = get_cloudfront_url(s3_key)
            except Exception as e:
                logging.error(f"Error generating URL for {s3_key}: {e}")
                continue

            # Extract report type from filename
            # "Broker_Payroll_Summary.pdf" -> "Payroll Summary"
            base_name = filename.replace(".pdf", "")
            if base_name.startswith("Broker_"):
                report_type = base_name.replace("Broker_", "").replace("_", " ").title()
            else:
...
#
# Xero document handling
#



def update_anonymized_json_xero(
    hashed_email: str,
    parent_header: str,
    sibling_header: list[str],
    org_name: str,
) -> bool:
    """
    Updating anonymized json for myob and xero
    ...
    hashed_email (str): The hashed email identifier
    parent_header (str): The parent header in the JSON structure
    sibling_header (list[str]): List of filenames to add
    ...
    # Convert list of filenames to dict with empty string values
    files_dict = {filename: "" for filename in sibling_header}
...
def get_docs_xero(client_id: str, emails: list, category: str) -> list:
    """
    Get Xero documents for a specific organization.
    Category format: organization name (e.g., "Demo Company (AU)")
    """
    if not verify_client_by_id(client_id):
        raise HTTPException(status_code=403, detail="Invalid client")
    ...
    for email_entry in emails:
        ...
        try:
            s3_objects = s3.list_objects_v2(
                Bucket=bucket_name, Prefix=hashed_email + "/" + prefix
            )
            files = s3_objects.get("Contents", [])
            ...
            if doc_type_keys:
                ...
                docs.append(
                    {
                        "id": f"{hashed_email}_{category}",
                        "category": category_label,
                        "category_data": {
                            category_label: doc_category_name
                        },
                        "url": urls,
                        "hashed_email": hashed_email,
                        "broker_comment": broker_comment,
            ...
            logging.error(f"Error fetching {prefix} for {hashed_email}: {e}")
            continue
...
def get_client_verified_ids_documents(
    client_id: str, emails: list, category: str
) -> list:
    """
                ...
                all_verified_docs.append(
                    {
                        ...
                        "category_data": {"Identity Verification": category},
                        "url": urls,
                        "hashed_email": hashed_email,
                    }
                )
...
async def upload_client_document(
    client_email: str, category: str, category_data: dict, file: UploadFile
) -> dict:
    """
...
def add_comment_docs_general(
    ...
    if updated:
        ...
        raise HTTPException(
            status_code=404, detail=f"Document with category '{category}' not found"
        )
...
def add_comment_docs_myob(
    ...
    try:
        ...
        for doc in documents:
            myob_reports = doc.get("myob_reports", {})
            if file_name in myob_reports:
                myob_reports[file_name] = comment
                updated = True
                logging.info(f"✓ Updated comment for {file_name}")
                break
...
def remove_comment_docs_general(
    client_id: str, hashed_email: str, category: str, parent_header: str
) -> None:
    """
...
def remove_comment_docs_xero(
    client_id: str, hashed_email: str, category: str, parent_header: str
) -> None:
    """
    Removes a broker comment from a Xero document in the anonymized JSON.
...
def remove_comment_docs_myob(
    client_id: str, hashed_email: str, filename: str, parent_header: str
) -> None:
    """
    Removes a broker comment from a Xero document in the anonymized JSON.
    ...
    try:
        ...
        for doc in documents:
            myob_reports = doc.get("myob_reports", {})
            if filename in myob_reports:
                myob_reports[filename] = ""
                updated = True
                logging.info(f"✓ Removed comment for {filename}")
                break
        ...
        logging.error(f"Error removing comment from MYOB document: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
def delete_client_document(hashed_email: str, threadid: str) -> None:
    ...
    documents = get_json_file(hashed_email, "/broker_anonymized/emails_anonymized.json")
    doc_index = next(
        (i for i, d in enumerate(documents) if d.get("threadid") == threadid), None
    )
...
def delete_client_document_identity(doc_name: str, hashed_email: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to delete identity documents"
        )
...
def delete_docs_xero(report_name: str, hashed_email: str):
    ...
    report_name (str): The sanitized filename (e.g., "GNN_and_Co_Pty_Ltd_xero_accounts_report.pdf")
    hashed_email (str): The hashed email identifier
...
def delete_docs_myob(report_name: str, hashed_email: str):
    """
    Delete a Xero document from S3 and emails_anonymized.json.
    ...
    try:
        # Construct the S3 key
        anonymized_key = "/broker_anonymized/emails_anonymized.json"
        report_key = f"{hashed_email}/myob_reports/{report_name}"
        ...
        for doc in documents:
            myob_reports = doc.get("myob_reports", {})
            if report_name in myob_reports:
                del myob_reports[report_name]
                logging.info(f"✓ Removed {report_name}")
                modified = True
                break
        ...
        logging.error(f"Error deleting MYOB document: {e}")
        raise HTTPException(status_code=500, detail=str(e))

# ------------------------
# Move Documents
# ------------------------
def move_pdfs_to_new_category(
    hashed_email: str, threadid: str, old_category: str, new_category: str
) -> None:
    """
...
def edit_client_document(hashed_email: str, update_data: dict) -> dict:
    ...
    doc_index = next(
        (i for i, d in enumerate(documents) if d.get("threadid") == card_id), None
    )
    if doc_index is None:
        raise HTTPException(
            status_code=404, detail=f"Document with id '{card_id}' not found"
        )
...
def get_download_urls(hashed_email: str, category: str, threadid: str) -> str:
    ...
    if not matched_urls:
        raise FileNotFoundError(
            f"No PDFs found for threadid '{threadid}' in category '{category}'"
        )
