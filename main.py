...
class Database_Retrieve:
    ...
    def check_and_retrieve_batches(self):
        """
        Check if there are pending batches at user_key/broker_batches/batches.json
        Returns the batches if they exist, None otherwise
        """
        try:
            print(f"[S3 BATCH CHECK] Checking for pending batches at {self.batch_path}")
            s3_object = self.s3_client.get_object(Bucket=self.bucket_name, Key=self.batch_path)
            body = s3_object['Body'].read()
            ...
            try:
                decompressed_body = gzip.decompress(body)
                data = json.loads(decompressed_body.decode('utf-8'))
                print(f"[S3 BATCH CHECK] Found compressed pending batches")
            except (OSError, gzip.BadGzipFile):
                # Not gzipped, try plain JSON
                data = json.loads(body.decode('utf-8'))
                print(f"[S3 BATCH CHECK] Found uncompressed pending batches")
            
            # Check if data has content
            if data and len(data) > 0:
                print(f"[S3 BATCH CHECK] Retrieved {len(data)} pending batch items")
                return data
            else:
                print(f"[S3 BATCH CHECK] Batch file exists but is empty")
                return None
                
        except self.s3_client.exceptions.NoSuchKey:
            print(f"[S3 BATCH CHECK] No pending batches found at {self.batch_path}")
            return None
        except Exception as e:
            print(f"[S3 BATCH CHECK] Error checking for batches: {e}")
            return None

    def save_pending_batches(self, emails_to_process):
        """
...
def handle_new_entry_broker(user_email):
    ...
    while True:
        ...
        # STEP 1: Check for pending batches first
        pending_batches = db_function.check_and_retrieve_batches()
        
        if pending_batches:
            print(f"[BATCH PROCESSING] Found pending batches to process")
            
            # Extract metadata if it exists
            if isinstance(pending_batches, dict) and '_batch_metadata' in pending_batches:
                # Extract the original batch filename from metadata
                current_batch_filename = pending_batches['_batch_metadata'].get('original_batch_filename')
                
                # CRITICAL FIX: Validate current_batch_filename is not None/empty
                if current_batch_filename and isinstance(current_batch_filename, str) and current_batch_filename.strip():
                    processing_new_batch = True
                    print(f"[BATCH PROCESSING] Continuing batch: {current_batch_filename}")
                else:
                    # Invalid or missing batch filename - corrupted metadata
                    print(f"[SAFETY] Pending batch has invalid/missing batch filename: {current_batch_filename}")
                    print(f"[SAFETY] Clearing corrupted pending batch to prevent infinite loop")
                    db_function.clear_pending_batches()
                    continue  # Skip to next iteration of while loop
                
                # Extract the actual emails from the structure
                emails = pending_batches.get('emails', pending_batches)
            else:
                # Legacy format or malformed data - must clear to prevent infinite loop
                print(f"[BATCH PROCESSING] WARNING: Pending batches missing metadata (legacy/malformed)")
                print(f"[SAFETY] Clearing orphaned pending batch to prevent infinite loop")
                
                # Try to extract emails, but clear pending batches regardless
                emails = pending_batches if isinstance(pending_batches, list) else []
                db_function.clear_pending_batches()
                
                # If no emails, skip this iteration
                if not emails:
                    print(f"[SAFETY] Orphaned pending batch was empty, moving to next batch")
                    continue
        else:
            print("[BATCH PROCESSING] No pending batches, retrieving next batch file")
            # STEP 2: If no pending batches, retrieve next unprocessed batch file
            ...
            if emails:
                ...
                original_emails_dict = emails.copy()
        ...
        if not emails:
            ...
            # CRITICAL: If we got here from pending_batches, clear them
            if pending_batches:
                print("[SAFETY] Clearing empty pending batch to prevent infinite loop")
                db_function.clear_pending_batches()
        ...
        llm = ChatOpenAI(temperature=0, model_name="gpt-4o-mini", api_key=API_KEY_OPENAI)
        ...
        if threads_json and not pending_batches:
            ...
            # For pending batches, use the emails directly as response
            response = pending_batches.get('emails', pending_batches) if isinstance(pending_batches, dict) else pending_batches

        if threads_json:
            # Process with time limit
            results_subject, subjects_to_process = asyncio.run(chunked_subject_batch(response, structured_llm_2, start_time))
            print("output of subject analysis")
            print(results_subject)
            response = filter_response_on_subject_output(results_subject, response)
            results, emails_to_process = asyncio.run(chunked_emails_true_batch(response, structured_llm, start_time))
        ...
        # Only update all_processed_emails if we have original email format (not from pending batches)
        if original_emails_dict:
            all_processed_emails.update(original_emails_dict)
        ...
        if None:
            ...
            db_function.save_pending_batches(pending_data)
            ...
            # Clear any pending batches if we processed everything
            if pending_batches:
                db_function.clear_pending_batches()
                print("[BATCH PROCESSING] Cleared pending batches")
