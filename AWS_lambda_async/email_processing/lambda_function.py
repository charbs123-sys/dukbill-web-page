...
def lambda_handler(event, context):
    """
    Lambda handler that runs async email collection.
    AWS Lambda supports async handlers by wrapping with asyncio.run()
    """
    return asyncio.run(async_lambda_handler(event, context))

async def async_lambda_handler(event, context):
    """
    Async version of the lambda handler
    """
    ...
    for message in event['Records']:
        ...
        # Process the threads asynchronously
        threads_yet_to_process = await email_collection.main_async(
            thread_ids, 
            user_token, 
            user_email_hash
        )
        print(f"Threads yet to process: {threads_yet_to_process}")
